...
public class SftpConfig {
	...
	@Value("${sftp.remote.directory.download:/upload}")
	private String sftpRemoteDirectoryDownload;

	@Value("${sftp.local.directory.download:localDownload}")
	...
	@Bean
	@InboundChannelAdapter(channel = "sftp", poller = @Poller(cron = "0/5 * * * * *"))
	...
	@Bean
    @ServiceActivator(inputChannel = "sftp")
    public MessageHandler handler() {
        SftpMessageHandler handler = new SftpMessageHandler(sftpSessionFactory());
        handler.setRemoteDirectoryExpression(new LiteralExpression(sftpRemoteDirectoryDownload));
